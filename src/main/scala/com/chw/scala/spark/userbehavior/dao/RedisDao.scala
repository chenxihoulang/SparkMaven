...
import com.chw.scala.spark.userbehavior.util.{Conf, RedisUtil, Tool}
...
object RedisDao {
  ...
  def updateRedis(_records: Seq[Conf.DT]) {
    ...
    val records: Seq[(String, ArrayBuffer[(Long, Long)])] = _records.map {
      case (uid, updates) =>
        (constructKey(uid), updates)
    }

    val uids: Seq[String] = records.map(_._1)
    val rawVals: ArrayBuffer[Array[Byte]] = RedisUtil.batchGet(uids)
    val recordAndVal: Seq[((String, ArrayBuffer[(Long, Long)]), Array[Byte])] = records.zip(rawVals)
